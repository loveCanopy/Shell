...
def bduss_decode(bduss):
    ...
    for letter in bduss:
        ...
        pos += 1
    salt = salt%2 + 1
    ret = struct.unpack("I", base64.b64decode(bduss[(-salt):] + bduss[:(-salt)].replace("-", "+").replace("~", "/"))[68:72])
    if len(ret) == 0:
        return 0
...
class conf(object):
    ...
    length= 8
    #正则匹配规则
    p = re.compile(r"^(\d+\.\d+\.\d+\.\d+) .*?\[(.*?)/(.*?)/(.*?):(\d+):(\d+):(\d+) (.*?)\] \{\"data\":\[\"(.*?)\"\]\}$")
...
class matchLog(object):
    ...
    def splitMap(self, logStr):
        keyValueList = []
        if logStr and len(logStr) > 0:
            listLog = logStr.split('&')
            for i in listLog:
                elementList = i.split('=')
